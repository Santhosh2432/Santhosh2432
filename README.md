<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:e8a15c,100:5ad4e6&height=220&section=header&text=Santhosh%20Kumar%20M.R&fontSize=46&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=AI%20Engineer%20and%20Full%20Stack%20Developer&descAlignY=58&descSize=20" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&size=22&duration=3000&pause=900&color=5AD4E6&center=true&vCenter=true&width=650&lines=Building+AI-powered+systems+that+ship.;RAG+pipelines+%7C+Agentic+AI+%7C+Computer+Vision;React+%C2%B7+FastAPI+%C2%B7+Flutter+%C2%B7+Azure;Completed%3A+AI+and+Drone+Intern+%40+Garuda+Aerospace" alt="Typing SVG" />
</a>

<br/>

<a href="mailto:santhoshkumar243276@gmail.com"><img src="https://img.shields.io/badge/Email-e8a15c?style=for-the-badge&logo=gmail&logoColor=white"/></a>
<a href="https://www.linkedin.com/in/santhosh-kumar-738b83275"><img src="https://img.shields.io/badge/LinkedIn-5ad4e6?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
<a href="https://santhosh2432.github.io"><img src="https://img.shields.io/badge/Portfolio-08090b?style=for-the-badge&logo=firefox&logoColor=e8a15c"/></a>
<a href="tel:+919344798857"><img src="https://img.shields.io/badge/Phone-08090b?style=for-the-badge&logo=whatsapp&logoColor=5ad4e6"/></a>

</div>

<br/>

## About Me

```yaml
name: "Santhosh Kumar M.R"
role: "AI Engineer & Full Stack Developer"
location: "Chengalpattu, Tamil Nadu, India"
focus: ["RAG Pipelines", "Agentic AI Systems", "Computer Vision", "Full-Stack AI Products"]
currently_building: "AI-powered web & mobile apps with React, FastAPI, Flutter & Azure"
open_to: "AI Engineering / Full-Stack roles & freelance projects"
```

Aspiring AI Engineer and Full-Stack Developer with hands-on experience building AI-powered web and mobile applications. Strong interest in RAG pipelines, agentic AI systems, and practical, scalable AI-driven products — from model training to shipped UI.

<br/>

## Experience

<table>
<tr>
<td width="90">
<img src="https://cdn-icons-png.flaticon.com/512/2103/2103658.png" width="60"/>
</td>
<td>

**AI & Drone Intern** · [Garuda Aerospace Limited](https://www.garudaaerospace.com/), Chennai
`Feb 2026 — May 2026`

- Trained custom **YOLOv8** object detection models on annotated datasets (Google Colab) for an autonomous wall-painting drone project
- Built AI-powered object detection & drone path-planning simulation for autonomous surface identification and painting trajectory navigation
- Optimized dataset preprocessing, annotation, and training workflows for real-time detection accuracy and UAV navigation integration

</td>
</tr>
</table>

<br/>

## Projects

<table>
<tr>
<td width="50%" valign="top">

### 🧠 [AI Automated Infographic Video Generator](https://github.com/Santhosh2432/AI_Automated_Infographic-Advertisment_Video_Generator)
Generates infographic videos automatically from raw documents using Azure OpenAI, DALL·E & AI Speech.

`Python` `FastAPI` `React.js` `Azure AI` `OAuth`

</td>
<td width="50%" valign="top">

### 💪 [Saiyan Fit — AI Fitness App](https://github.com/Santhosh2432/Saiyan_fit)
Full-stack AI fitness app with real-time coaching via Groq API (Llama 3), offline-first support.

`Flutter` `FastAPI` `MongoDB` `Azure Blob` `Groq API`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### 📄 DocAgent
Full-stack RAG system with Groq Llama 3.3 70B + MiniLM embeddings for source-grounded QA, with MCP-powered agentic workflows.

`MCP` `RAG` `FastAPI` `React.js` `Groq API`

</td>
<td width="50%" valign="top">

### 🏗️ [CrackScope](https://github.com/Santhosh2432/CrackScope)
AI-powered structural inspection platform — custom YOLOv8 models hitting 91% mAP@50 for crack & window detection.

`YOLOv8` `Google Colab` `FastAPI` `React.js` `OpenCV`

</td>
</tr>
</table>

<br/>

## Tech Stack

<div align="center">

<img src="https://skillicons.dev/icons?i=java,py,js,react,flutter,nodejs,fastapi,mongodb,postgres,azure,git,github&theme=dark" />

<br/><br/>

![MCP](https://img.shields.io/badge/MCP-08090b?style=flat-square&logoColor=5ad4e6)
![RAG](https://img.shields.io/badge/RAG-08090b?style=flat-square&logoColor=e8a15c)
![LLM](https://img.shields.io/badge/LLM-08090b?style=flat-square&logoColor=5ad4e6)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat-square&logo=openai&logoColor=white)
![Roboflow](https://img.shields.io/badge/Roboflow-6706CE?style=flat-square&logo=roboflow&logoColor=white)
![Google Colab](https://img.shields.io/badge/Colab-F9AB00?style=flat-square&logo=googlecolab&logoColor=white)

</div>

<br/>

## GitHub Stats

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Santhosh2432&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=5ad4e6&icon_color=e8a15c&text_color=c9d1d9" />
<img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Santhosh2432&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=5ad4e6&text_color=c9d1d9" />

<br/>

<img src="https://streak-stats.demolab.com?user=Santhosh2432&theme=tokyonight&hide_border=true&background=0D1117&ring=e8a15c&fire=5ad4e6&currStreakLabel=5ad4e6" />

<br/>

<img src="https://github-profile-trophy.vercel.app/?username=Santhosh2432&theme=tokyonight&no-frame=true&margin-w=12&column=7" />

</div>

<br/>

## Education & Recognition

**Agni College of Technology** — B.E, Computer Science & Engineering `2022 — 2026`
📈 CGPA: **8.54 / 10**  ·  🏆 2nd Prize — Intra College Project Presentation

**Certifications:** AI Fluency Course · Introduction to Model Context Protocol · Building with Claude API — *Anthropic Academy*  ·  Python & Java Beginner — *Udemy*  ·  Campus to Technical Careers (SQL, Core Java 8, Spring Boot, Angular.js) — *Capgemini TNS Foundation*

<br/>

<div align="center">

### Let's build something worth shipping.

<a href="mailto:santhoshkumar243276@gmail.com"><img src="https://img.shields.io/badge/Get%20In%20Touch-e8a15c?style=for-the-badge&logoColor=white"/></a>

<br/><br/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:5ad4e6,100:e8a15c&height=100&section=footer" width="100%"/>

</div>
